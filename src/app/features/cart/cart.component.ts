...
@Component({
  ...
  template: `
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <app-breadcrumb [breadcrumbs]="[{ label: 'Cart', path: '' }]" />

      <h1 class="text-3xl font-bold tracking-tight text-gray-900">Cart</h1>

      @if (store.cartItems().length === 0) {
        <div class="flex flex-col items-center justify-center py-20 text-center">
          <svg class="mb-4 h-16 w-16 text-gray-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
        ...
        <div class="mt-8 space-y-4">
          @for (item of store.cartItems(); track item.productId + item.colorCode + item.storageCode) {
            <div class="flex items-center gap-4 rounded-xl border border-gray-100 p-4 transition-all hover:border-gray-200">
              <div class="h-20 w-20 shrink-0 overflow-hidden rounded-lg bg-gray-50">
                <img [src]="item.imageUrl" [alt]="item.productName" class="h-full w-full object-contain p-2" />
              </div>

              <div class="min-w-0 flex-1">
                <p class="text-xs font-medium uppercase tracking-wider text-gray-400">{{ item.brand }}</p>
                ...
                <div class="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-gray-500">
                  <span class="flex items-center gap-1">
                    <span class="h-2.5 w-2.5 rounded-full" [style.background-color]="getColorHex(item.colorCode)"></span>
                    {{ item.colorName }}
                  </span>
              ...
              <div class="flex items-center gap-3">
                <div class="flex items-center rounded-lg border border-gray-200">
                  <button
                    (click)="store.updateQuantity(item.productId, item.colorCode, item.storageCode, -1)"
                  ...
                  <span class="min-w-[2ch] text-center text-sm font-medium text-gray-900">{{ item.quantity }}</span>
                  <button
                    (click)="store.updateQuantity(item.productId, item.colorCode, item.storageCode, 1)"
                    class="px-2.5 py-1.5 text-gray-500 transition-colors hover:text-gray-900"
...
export default class CartComponent {
  ...
  getColorHex(code: string): string {
    const colors: Record<string, string> = {
      CBLK: '#1a1a1a', CWH: '#f0f0f0', CNAT: '#8b8b7a', CDES: '#c4a882',
      CTI: '#6b6b72', CTB: '#4a6fa5', CTW: '#e8e8e8', CMI: '#98c5b0',
      CLA: '#c4b5d4', CGR: '#4a4a4a', COB: '#1c1c1e', CPO: '#f5f0e8',
      CHA: '#8b8b7a', CRO: '#e8b4b8', CSK: '#b8d4e8', CBL: '#1a1a1a',
      CGW: '#e8e8f0', CSR: '#0a0a0a', CMB: '#2d4a7a',
    };
